...
projectController.getProjects = (req, res, next) => {
  ...
  try {
    ...
    if (res.locals.isLoggedIn) {
      ...
      const projects = db.query(queryString);
...
projectController.addProject = (req, res, next) => {
  ...
  const { username } = res.locals;
  const { projName, frontend, css, runtime, buildtool, database, unit, e2e } = req.body.projectList;
  ...
  try {
    ...
    if (res.locals.isLoggedIn) {
      ...
      const value = [username, projName, frontend, css, runtime, buildtool, database, unit, e2e];
